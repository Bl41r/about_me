...
'use strict';
// Global Variables
var tally = 0;  // number of questions correct
...
function playGuessingGame(num) {
  ...
  do {
    try {
      ...
      console.log('error, guess of: ' + guess + 'set to empty string');
      guess = '';
    }
    if (guess === num) {
...
function questions1thru5(){
  ...
  for (var q = 0; q < qArray.length; q++) {
    ...
    if ((answer === 'yes') || (answer === 'y') || (answer === 'yes.')) {
      ...
      alert('Please give a valid yes or no response.  Try again.');
...
// and alerts the user to possible correct answers
  var answerString = '';
